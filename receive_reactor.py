...
class NRF24Radio(NRF24):
    ...
    def __init__(self):
        self._log = Logger(observer=buildLogger())
...
class SensorAggregationController:
    ...
    def listenForData(self):
        ...
        uid = str(uuid.uuid1())
        self._log.info(uid + ": listenForData start")
	...
	datum.process()
        ...
        self._log.info(uid + ": listenForData end")
...
class SensorDatumProcessor:
    ...
    @inlineCallbacks
    def process(self):
	self._log.info(self._uid + ": process start")
	...
	self._log.info(self._uid + ": process end")
...
class NRF24DataTransformer:
    ...
    def convertBufferToUnicode(self, buffer):
        self._log.debug(self._uid + ": convertBufferToUnicode")
	self._log.debug("Buffer: {buffer}", buffer=buffer)
...
class SensorDataParser:
    ...
    def convertMessageToPostBody(self, message):
        self._log.debug(self._uid + ": convertMessageToPostBody")
        def splitter(message):
            if message.count('::') > 0:
                words = message.split("::")
                self._log.debug(self._uid + ": split message: " + str(words))
...
class SensorDataBroadcaster:
    ...
    def broadcast(self, postBody):
        self._log.debug(self._uid + ": postMessageToServer")
    ...
    def processResponses(self, response):
        self._log.debug(self._uid + ": processServerResponse")
        json = yield response.json();
	defer.execute(self._printResponse, json)
        returnValue(json)

    def _printResponse(self, responseJson):
        self._log.debug(self._uid + ": " + "Response: " + json.dumps(responseJson, sort_keys=True,
                                                        indent=4, separators=(',', ': ')))
